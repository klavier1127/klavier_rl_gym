...
class AMPOnPolicyRunner:
    def __init__(self, env: VecEnv, train_cfg, log_dir=None, device="cpu"):
        ...
        discriminator = AMPDiscriminator(amp_data.observation_dim * 2, train_cfg['runner']['amp_reward_coef'], device).to(self.device)
    ...
    def learn(self, num_learning_iterations, init_at_random_ep_len=False):
        ...
        obs, critic_obs, privileged_obs, obs_history, amp_obs = obs.to(self.device), critic_obs.to(self.device), privileged_obs.to(self.device), obs_history.to(self.device), amp_obs.to(self.device)
        ...
        for it in range(self.current_learning_iteration, tot_iter):
            ...
            with torch.inference_mode():
                for i in range(self.num_steps_per_env):
                    ...
                    privileged_obs, obs_history, next_amp_obs = infos['privileged_obs'], infos['obs_history'], infos['amp_obs']
                    obs, critic_obs, rewards, dones, privileged_obs, obs_history, amp_obs = obs.to(self.device), critic_obs.to(self.device), rewards.to(self.device), dones.to(self.device), privileged_obs.to(self.device), obs_history.to(self.device), next_amp_obs.to(self.device)
                    # Account for terminal states.
                    next_amp_obs_with_term = torch.clone(next_amp_obs)
                    reset_env_ids, terminal_amp_states = infos['reset_env_ids'], infos['terminal_amp_states']
                    next_amp_obs_with_term[reset_env_ids] = terminal_amp_states

                    rewards += self.alg.discriminator.predict_amp_reward(amp_obs, next_amp_obs_with_term, rewards, normalizer=self.alg.amp_normalizer)
