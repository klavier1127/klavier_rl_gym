...
from legged_gym.algo.ppo_amp.datasets import pose3d
from legged_gym.algo.ppo_amp.datasets import motion_util
...
class AMPLoader:
    ...
    POS_SIZE = 3  # foot_pos
    # ROT_SIZE = 4  # foot_quat
    JOINT_POS_SIZE = 10  # dof_pos
    LINEAR_VEL_SIZE = 3  # lin_vel
    ANGULAR_VEL_SIZE = 3  # ang_vel
    JOINT_VEL_SIZE = 10  # dof_vel
    ROOT_POS_SIZE = 1  # base height
    ROOT_ROT_SIZE = 2  # euler[:, :2]
    ...
    ANGULAR_VEL_START_IDX = JOINT_VEL_END_IDX
    ANGULAR_VEL_END_IDX = ANGULAR_VEL_START_IDX + ANGULAR_VEL_SIZE

    ROOT_ROT_START_IDX = ANGULAR_VEL_END_IDX
    ...
    ROOT_POS_START_IDX = LINEAR_VEL_END_IDX
    ROOT_POS_END_IDX = ROOT_POS_START_IDX + ROOT_POS_SIZE

    FOOT_POS_START_IDX = ROOT_POS_END_IDX
        ...
        for i, motion_file in enumerate(motion_files):
            ...
            with open(motion_file, "r") as f:
                ...
                for f_i in range(motion_data.shape[0]):
                    ...
                    motion_data[f_i, AMPLoader.POS_SIZE:(AMPLoader.POS_SIZE + AMPLoader.ROOT_ROT_SIZE)] = root_rot
    ...
    def reorder_from_pybullet_to_isaac(self, motion_data):
        ...
        jp_r, jp_l = np.split(
            AMPLoader.get_joint_pose_batch(motion_data), 2, axis=1)
        joint_pos = np.hstack([jp_l, jp_r])

        fp_r, fp_l = np.split(
            AMPLoader.get_tar_toe_pos_local_batch(motion_data), 2, axis=1)
        foot_pos = np.hstack([fp_l, fp_r])

        lin_vel = AMPLoader.get_linear_vel_batch(motion_data)
        ang_vel = AMPLoader.get_angular_vel_batch(motion_data)

        jv_r, jv_l = np.split(
            AMPLoader.get_joint_vel_batch(motion_data), 2, axis=1)
        joint_vel = np.hstack([jv_l, jv_r])

        fv_r, fv_l = np.split(
            AMPLoader.get_tar_toe_vel_local_batch(motion_data), 2, axis=1)
        foot_vel = np.hstack([fv_l, fv_r])
