...
from legged_gym.algo.ppo_lpd.estimator import PrivilegedEncoder, MLPHistoryEncoder
...
class ActorCritic(nn.Module):
    ...
    def act_student(self, observations, obs_history, masks=None, hidden_states=None):
        ...
        with torch.no_grad():
            input_memory = torch.cat((observations, latent), dim=-1)
            input_a = self.memory_a(input_memory, masks, hidden_states)
        actions_mu_stu = self.actor_student(input_a.squeeze(0))
        return actions_mu_stu

    def act_inference(self, observations, privileged_obs, obs_history):
        latent = self.estimator(obs_history)
        input_memory = torch.cat((observations, latent), dim=-1)
