...
class AMPDiscriminator(nn.Module):
    ...
    def predict_amp_reward(
            self, last_state, state, task_reward, normalizer=None):
        with torch.no_grad():
            self.eval()
            if normalizer is not None:
                last_state = normalizer.normalize_torch(last_state, self.device)
                state = normalizer.normalize_torch(state, self.device)
            d = self.amp_linear(self.trunk(torch.cat([last_state, state], dim=-1)))
            amp_reward = self.amp_reward_coef * torch.clamp(1 - (1/4) * torch.square(d - 1), min=0)
            self.train()
        return amp_reward.squeeze(), d
