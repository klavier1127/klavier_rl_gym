...
class PPO:
    ...
    def init_storage(self, num_envs, num_transitions_per_env, actor_obs_shape, critic_obs_shape, privileged_obs_shape, obs_history_shape, single_obs_shape, action_shape):
        self.storage = RolloutStorage(num_envs, num_transitions_per_env, actor_obs_shape, critic_obs_shape, privileged_obs_shape, obs_history_shape, single_obs_shape, action_shape, self.device)
    ...
    def act(self, obs, critic_obs, privileged_obs, obs_history):
        ...
        self.transition.values = self.actor_critic.evaluate(critic_obs, obs_history).detach()
    ...
    def compute_returns(self, last_critic_obs, obs_history):
        last_values= self.actor_critic.evaluate(last_critic_obs, obs_history).detach()
    ...
    def update(self):
        ...
        for (obs_batch, critic_obs_batch, privileged_obs_batch, obs_history_batch, next_obs_batch, actions_batch,
             target_values_batch, advantages_batch, returns_batch, old_actions_log_prob_batch,
                ...
                value_batch = self.actor_critic.evaluate(critic_obs_batch, obs_history_batch)
                ...
                vel = privileged_obs_batch[..., :3]
                self.vae_optimizer.zero_grad()
                vae_loss = self.actor_critic.vae.loss_fn(obs_history_batch, next_obs_batch, vel)
                vae_loss.backward()
