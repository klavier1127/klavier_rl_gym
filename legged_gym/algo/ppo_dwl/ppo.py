...
class PPO:
    ...
    def update(self):
        ...
        for (
            ...
            # DWL loss
            # Reconstructed loss
            input_a, reconstructed_a = self.actor_critic.memory_a(obs_batch, masks=masks_batch, hidden_states=hid_states_batch[0])
            input_c, _ = self.actor_critic.memory_c(critic_obs_batch, masks=masks_batch, hidden_states=hid_states_batch[1])
            reconstructed_loss = torch.nn.MSELoss()(reconstructed_a, input_c.detach())
            dwl_loss = reconstructed_loss

            loss = surrogate_loss + self.value_loss_coef * value_loss - self.entropy_coef * entropy_batch.mean() + dwl_loss
            ...
            mean_dwl_loss += dwl_loss.item()

        num_updates = self.num_learning_epochs * self.num_mini_batches
        mean_value_loss /= num_updates
        mean_surrogate_loss /= num_updates
        mean_dwl_loss /= num_updates
        self.storage.clear()

        return mean_value_loss, mean_surrogate_loss, mean_dwl_loss
