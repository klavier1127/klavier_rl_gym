...
from legged_gym.algo.ppo_lpd.estimator import PrivilegedEncoder, Regulator, MLPHistoryEncoder
...
class Memory(torch.nn.Module):
    def __init__(self, input_size, output_size, type="lstm", num_layers=1, hidden_size=128, decoder_hidden_dims=[256, 128], activation = nn.ELU()):
        super().__init__()
        # RNN
        ...
        decoder_layers = []
        decoder_layers.append(nn.Linear(hidden_size, decoder_hidden_dims[0]))
        decoder_layers.append(activation)
        for l in range(len(decoder_hidden_dims)):
            if l == len(decoder_hidden_dims) - 1:
                decoder_layers.append(nn.Linear(decoder_hidden_dims[l], output_size))
            else:
