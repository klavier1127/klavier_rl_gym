...
class EstimatorNet(nn.Module):
    def __init__(self,num_obs_history):
        ...
        self.latent_priv = nn.Linear(256, 6)
        self.lantent_env = nn.Linear(256, 1)

    def forward(self, obs_history):
        encoded = self.encoder(obs_history)
        latent_priv = self.latent_priv(encoded)
        latent_env = self.lantent_env(encoded)
        return latent_priv, latent_env
