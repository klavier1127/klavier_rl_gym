...
from legged_gym.envs.x02_2.x02_2_config import x02_2Cfg
...
def run_mujoco(policy, cfg):
    ...
    default_pos = [0, 0, 0.3, -0.7, 0.4, 0,     0, 0, 0.3, -0.7, 0.4, 0]
...
if __name__ == '__main__':
    ...
    class Sim2simCfg(x02_2Cfg):
        class sim_config:
            mujoco_model_path = f'{LEGGED_GYM_ROOT_DIR}/resources/robots/X02A_2/scene.xml'
            sim_duration = 60.0
            dt = 0.001
            decimation = 20
        class robot_config:
            kps = np.array([200, 200, 200, 200, 50, 50,     200, 200, 200, 200, 50, 50], dtype=np.double)
            kds = np.array([4, 4, 4, 4, 1, 1,                4, 4, 4, 4, 1, 1], dtype=np.double)
            tau_limit = 50000. * np.ones(12, dtype=np.double)


    model_path = "../logs/x02_2/exported/policies/policy_pia.pt"
