...
from legged_gym.envs import x02Cfg
import torch

from legged_gym.envs.x02_run.x02_run_config import x02runCfg
...
if __name__ == '__main__':
    import argparse
    class Sim2simCfg(x02runCfg):
        class sim_config:
            # mujoco_model_path = f'{LEGGED_GYM_ROOT_DIR}/resources/robots/x021_scence/scene_star.xml'
            mujoco_model_path = f'{LEGGED_GYM_ROOT_DIR}/resources/robots/X02A/scene.xml'
            sim_duration = 60.0
            dt = 0.001
            decimation = 20
        class robot_config:
            kps = np.array([200, 200, 200, 200, 50, 200, 200, 200, 200, 50], dtype=np.double)
            kds = np.array([4, 4, 4, 4, 1, 4, 4, 4, 4, 1], dtype=np.double)
            tau_limit = np.array([50, 110, 72, 150, 45, 50, 110, 72, 150, 45], dtype=np.double)

    parser = argparse.ArgumentParser(description='Deployment script.')
    parser.add_argument('--terrain', action='store_true', help='terrain or plane')
    args = parser.parse_args()
    model_path = "../logs/x02_run/exported/policies/policy_mlp.pt"
