...
from legged_gym.envs.bx.bx_config import bxCfg
...
def run_mujoco(policy, cfg):
    ...
    default_pos = [0, 0, -0.3, 0.6, -0.3, 0,   0, 0, -0.3, 0.6, -0.3, 0]
...
if __name__ == '__main__':
    ...
    class Sim2simCfg(bxCfg):
        class sim_config:
            mujoco_model_path = f'{LEGGED_GYM_ROOT_DIR}/resources/robots/bx/scene.xml'
            sim_duration = 60.0
            dt = 0.001
            decimation = 20
        class robot_config:
            kps = 1 * np.array([150, 150, 150, 150, 20, 20, 150, 150, 150, 150, 20, 20], dtype=np.double)
            kds = np.array([3, 3, 3, 3, 1, 1, 3, 3, 3, 3, 1, 1], dtype=np.double)
            tau_limit = 50000. * np.ones(12, dtype=np.double)


    # model_path = "../logs/bx/exported/policies/policy_pia_example.pt"
    model_path = "../logs/bx/exported/policies/policy_pia.pt"
