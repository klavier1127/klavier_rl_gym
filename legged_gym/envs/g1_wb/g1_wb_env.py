...
class g1WBEnv(LeggedRobot):
    ...
    def _reward_waist_pos(self):
        error = self.sqrdexp(10. * (self.dof_pos[:, 12]))
        return error / 1.

    def _reward_arm_mirror(self):
        error = 0
        error += self.sqrdexp(10. * (2.0 * self.dof_pos[:, 13] + self.dof_pos[:, 6]))
        error += self.sqrdexp(10. * (2.0 * self.dof_pos[:, 18] + self.dof_pos[:, 0]))
        return error / 2.

    def _reward_shoulder_pos(self):
        error = 0
        error += self.sqrdexp(10. * (self.dof_pos[:, 16] - 1.5))
        error += self.sqrdexp(10. * (self.dof_pos[:, 21] - 1.5))
        return error / 2.

    def _reward_elbow_pos(self):
        error = 0
        error += self.sqrdexp(10. * (self.dof_pos[:, 14] - 0.2))
        error += self.sqrdexp(10. * (self.dof_pos[:, 19] + 0.2))
        return error / 2.

    def _reward_wrist_pos(self):
        error = 0
        error += self.sqrdexp(10. * (self.dof_pos[:, 17]))
        error += self.sqrdexp(10. * (self.dof_pos[:, 22]))
        return error / 2.
