...
class LeggedRobot(BaseTask):
    ...
    def _process_rigid_body_props(self, props, env_id):
        ...
        if self.cfg.domain_rand.randomize_all_com:
            for s in range(len(props) - 1):
                ...
                props[s].com.x += rand_com_x
                props[s].com.y += rand_com_y
                props[s].com.z += rand_com_z
        ...
        if self.cfg.domain_rand.randomize_base_com:
            ...
            rand_com_y = np.random.uniform(rng_com[0], rng_com[1])
            rand_com_z = np.random.uniform(rng_com[0], rng_com[1])
            props[0].com.x += rand_com_x
            props[0].com.y += rand_com_y
            props[0].com.z += rand_com_z
        base_com = torch.tensor([props[0].com.x, props[0].com.y, props[0].com.z], dtype=torch.float32)
        self.base_com[env_id] = base_com
    ...
    def _post_physics_step_callback(self):
        ...
        if self.cfg.domain_rand.randomize_commands:
            ...
            self.commands[:, 1] += 0.01 * (2 * torch.rand_like(self.commands[:, 1]) - 1)
    ...
    def _create_envs(self):
        ...
        self.base_com = torch.zeros(self.num_envs, 3, dtype=torch.float32, device=self.device, requires_grad=False)
    ...
    def _reward_contact_forces(self):
        ...
        error = torch.sum((contact_forces - self.cfg.rewards.max_contact_force).clip(0, 400), dim=1)
        return error
    ...
    def _reward_collision(self):
        collision = torch.norm(self.contact_forces[:, self.penalised_contact_indices, :], dim=-1) > 0.1
        penalty = torch.sum(collision.float(), dim=1)
        return penalty
    ...
    def _reward_action_rate(self):
        ...
        error3 = 0.1 * torch.sum(torch.square(self.actions), dim=1)
    ...
    def smooth_sqr_wave(self, phase):
        p = 2.*torch.pi*phase * 1. / self.cfg.rewards.cycle_time
