...
class xbotEnv(LeggedRobot):

    def __init__(self, cfg: LeggedRobotCfg, sim_params, physics_engine, sim_device, headless):
        super().__init__(cfg, sim_params, physics_engine, sim_device, headless)
        self.last_feet_z = -1.02
    ...
    def _reward_ankle_pos(self):
        ...
        error += self.sqrdexp(10. * (self.dof_pos[:, 5] - self.default_dof_pos[:, 5]))
        error += self.sqrdexp(5. * (self.dof_pos[:, 10] - self.default_dof_pos[:, 10]))
        error += self.sqrdexp(10. * (self.dof_pos[:, 11] - self.default_dof_pos[:, 11]))
        return error / 4.
    ...
    def _reward_feet_air_time(self):
        ...
        error1 = torch.sum((self.feet_air_time - self.cfg.rewards.target_air_time) * first_contact, dim=1)
        ...
        error2 = self.max_feet_air_time[:, 0] - self.max_feet_air_time[:, 1]
        error2 = torch.square(error2)
        return torch.abs(error1) + error2
    ...
    def _reward_feet_height(self):
        ...
        # rew_feetHeight = (self.feet_height - self.cfg.rewards.target_feet_height) > -0.01
        error = torch.sum((self.feet_height - self.cfg.rewards.target_feet_height) * ~contact_mask, dim=1)
    ...
    def _reward_base_height(self):
        ...
        base_height = self.root_states[:, 2] - (base_feet_heights - 0.04)
